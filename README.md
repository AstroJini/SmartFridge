...
| **김지현** |  |
| **김찬진** | 이번 프로젝트를 통해 Redis, RabbitMQ, WebSocket 등 새로운 기술을 직접 적용하며 개발 역량을 넓힐 수 있었다. 단순히 기능을 구현하는 데 그치지 않고, 문제를 분석하고 해결하는 과정에서 많은 배움을 얻었다.<br>
팀 단위로 진행하면서 협업의 중요성도 체감했다. 각자 맡은 기능을 개발했지만, 이슈를 공유하고 해결하는 과정에서 팀워크가 강화되었고, GitHub 협업은 코드 관리와 작업 효율을 높이는 데 큰 도움이 되었다.<br>
특히 초반 기획을 충분히 준비한 덕분에 개발 과정이 안정적으로 진행되었다. 요구사항을 명확히 정의하고 흐름을 설계한 결과, 큰 수정 없이 기능을 구현할 수 있었고 전체적인 완성도를 높일 수 있었다.<br>
아쉬운 점은 단위 테스트를 충분히 진행하지 못해 일정에 일부 차질이 생긴 것이다. 앞으로는 자동화된 테스트를 도입해 개발 효율성과 안정성을 동시에 강화하고 싶다. |
| **윤세진** |  |
