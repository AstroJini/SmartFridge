...
초기 단계에서 ERD와 Figma를 정교하게 설계했습니다.
이 덕분에 이후 Entity 구조 설계와 화면 구현을 매우 수월하게 진행할 수 있었고,
“기획의 완성도가 개발 효율을 얼마나 크게 좌우하는지”를 직접 체감했습니다.
명확한 데이터 흐름과 UI 시나리오를 사전에 정의함으로써, 불필요한 수정과 리팩토링을 최소화할 수 있었습니다.

🔹 예외 처리 및 도메인 구조 개선

개발 과정에서 가장 큰 도전은 예외 처리의 복잡성이었습니다.
이를 해결하기 위해 공통 Validator를 직접 설계하고 전역적으로 적용함으로써,
중복된 로직을 제거하고 코드 유지보수성을 크게 높였습니다.

또한 상위-하위 도메인을 동시에 개발하던 중 의존성 충돌 문제를 경험하며,
“상위 도메인을 우선 개발하고 공통 모듈을 선행 구현해야 한다”는 원칙을 체득했습니다.
이 경험은 이후의 시스템 설계와 모듈 분리 전략에 중요한 기준이 되었습니다.

🔹 실시간 통신 아키텍처 설계

실시간 기능 구현을 위해 WebSocket을 도입하여
서버와 클라이언트 간 양방향 통신 아키텍처를 직접 설계했습니다.

다만, 실시간 알림의 경우 서버에서 클라이언트로의 단방향 전송이 주 목적이었기 때문에,
SSE(Server-Sent Events) 방식을 채택했습니다.
이를 통해 불필요한 연결 오버헤드를 줄이고 서버 리소스를 효율적으로 관리하면서도
안정적이고 지연이 적은 실시간 알림을 구현할 수 있었습니다.

알림 시스템은 다음과 같이 구성했습니다:

사용자 이벤트(식품 등록, 댓글 작성 등) 발생 시 알림 객체를 생성 후 DB에 저장

Redis Pub/Sub을 이용해 알림 설정이 활성화된 사용자에게만 실시간 전송

스케줄러를 활용하여 유통기한 알림을 특정 시간에 자동 발송

현재는 클라이언트의 수신 성공 여부를 추적하는 피드백 루프가 없어
메시지 유실 가능성이 존재하지만,
향후 메시지 큐(RabbitMQ 등)를 도입하여 재전송 시스템을 구축할 계획입니다.

🔹 DevOps 자동화와 무중단 배포

개발 후반부에는 GitHub Actions를 이용한 CI/CD 파이프라인을 구축했습니다.
코드가 특정 브랜치에 푸시되면 자동으로 테스트, 빌드, Docker 이미지 생성,
ECR 레지스트리 푸시, 그리고 EKS(Kubernetes) 환경으로 무중단 배포가 이루어지도록 구성했습니다.

이 과정을 통해 DevOps 전반의 이해도와 실무 적용 능력을 높였고,
개발 생산성을 획기적으로 개선할 수 있었습니다.
자동화된 배포 환경은 코드 품질 관리와 운영 안정성 향상에 크게 기여했습니다.

🔹 기술적 성장과 개선점

Redis, RabbitMQ 등 새로운 기술 스택을 직접 적용하면서
단순 구현을 넘어 구조적 문제 해결 중심의 사고를 익혔습니다.

특히, 사전 기획과 설계의 중요성을 체감했고,
공통화된 모듈 설계와 자동화 파이프라인 구축을 통해 시스템 확장성을 확보했습니다.

아쉬운 점은 단위 테스트 작성에 충분한 시간을 투자하지 못해
일정에 일부 영향을 주었다는 것입니다.
다음 프로젝트에서는 테스트 자동화 비중을 높여 품질 중심의 개발 프로세스를 구축할 계획입니다.

또한, 문제 해결 과정을 체계적으로 문서화하지 못한 부분을 개선하여,
향후에는 기술 공유와 재사용성을 높이는 개발 문화를 정착시키고자 합니다.
<br/>
